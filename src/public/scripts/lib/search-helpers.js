  ...
  if (submitting) {
    ...
    if (error) {
      ...
      const foldersHTML = results.folders
        .map((folder) => generateSearchFolderItem(folder))
        .join('');

      const filesHTML = results.files
        .map((file) => generateSearchFileItem(file))
        .join('');
...
function generateSearchFileItem(file) {
  return `
    <div
...
function generateSearchFolderItem(folder) {
  return `
    <a
